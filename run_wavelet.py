...
def get_dataset(data_length=1000, sample_rate=2e-6):
    ...
    # Save data as CSV
    np.save("dataset.in.npy", in_tensors.numpy())
    np.save("dataset.out.npy", out_tensors.numpy())

    return torch.utils.data.TensorDataset(in_tensors, out_tensors)


def load_dataset(in_filename="dataset.in.npy", out_filename="dataset.out.npy"):
