...
def get_dataset(data_length=2000, sample_rate=2e-6):
    # Load CSV Files
    # TODO: Use numpy instead of pandas
    
    V = pd.read_csv('D:\\Dropbox (Princeton)\\DummyData\\N87\\VOLT.csv', header=None).iloc[:,0:data_length]
    # V = pd.read_csv('D:\Dropbox (Princeton)\MagNet_TestData_dummy\Dummy_volt.csv', header=None).iloc[:,0:data_length]
    
    P = pd.read_csv('D:\\Dropbox (Princeton)\\DummyData\\N87\\LOSS.csv', header=None)
    ...
    out_tensors = torch.from_numpy(P.to_numpy()).view(-1, 1)
...
def main():
    ...
    optimizer = optim.Adam(net.parameters(), lr=0.0017)
